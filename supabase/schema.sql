-- Full PostgreSQL conversion from TableQuanLyChiTieuCaNhan_v2.sql
-- Target: Supabase SQL Editor (PostgreSQL)

BEGIN;

-- Reset objects to allow re-run safely
...
-- Sequences
CREATE SEQUENCE seq_nguoidung START 1 INCREMENT 1;
CREATE SEQUENCE seq_vitien START 1 INCREMENT 1;
CREATE SEQUENCE seq_danhmuc START 1 INCREMENT 1;
CREATE SEQUENCE seq_giaodich START 1 INCREMENT 1;
CREATE SEQUENCE seq_ngansach START 1 INCREMENT 1;
CREATE SEQUENCE seq_muctieutaichinh START 1 INCREMENT 1;
CREATE SEQUENCE seq_baocaotaichinh START 1 INCREMENT 1;
...
ALTER TABLE nguoidung
  ALTER COLUMN nguoidung_id SET DEFAULT ('ND' || lpad(nextval('seq_nguoidung')::text, 2, '0'));
...
CREATE TABLE vitien (
  ...
  so_du_hien_tai numeric(19,2) NOT NULL,
...
ALTER TABLE vitien
  ALTER COLUMN vi_id SET DEFAULT ('Vi' || lpad(nextval('seq_vitien')::text, 2, '0'));
...
ALTER TABLE danhmuc
  ALTER COLUMN danhmuc_id SET DEFAULT ('DM' || lpad(nextval('seq_danhmuc')::text, 2, '0'));
...
CREATE TABLE giaodich (
  ...
  so_tien numeric(19,2) NOT NULL CHECK (so_tien > 0),
...
ALTER TABLE giaodich
  ALTER COLUMN giaodich_id SET DEFAULT ('GD' || lpad(nextval('seq_giaodich')::text, 2, '0'));

CREATE TABLE ngansach (
  ngansach_id varchar(10) PRIMARY KEY,
  ten_ngan_sach varchar(100) NOT NULL,
  so_tien_gioi_han numeric(19,2) NOT NULL CHECK (so_tien_gioi_han > 0),
  ngay_bat_dau date NOT NULL DEFAULT CURRENT_DATE,
  ngay_ket_thuc date NOT NULL DEFAULT (CURRENT_DATE + INTERVAL '30 days'),
  id_nguoi_dung varchar(10) NOT NULL,
  id_danh_muc varchar(10) NOT NULL,
  FOREIGN KEY (id_nguoi_dung) REFERENCES nguoidung (nguoidung_id),
  FOREIGN KEY (id_danh_muc) REFERENCES danhmuc (danhmuc_id),
...
ALTER TABLE ngansach
  ALTER COLUMN ngansach_id SET DEFAULT ('NS' || lpad(nextval('seq_ngansach')::text, 2, '0'));

CREATE TABLE muctieutaichinh (
  muctieu_id varchar(10) PRIMARY KEY,
  ten_muc_tieu varchar(100) NOT NULL,
  so_tien_can_dat numeric(19,2) NOT NULL CHECK (so_tien_can_dat > 0),
  ngay_bat_dau date NOT NULL DEFAULT CURRENT_DATE,
...
ALTER TABLE muctieutaichinh
  ALTER COLUMN muctieu_id SET DEFAULT ('MT' || lpad(nextval('seq_muctieutaichinh')::text, 2, '0'));

CREATE TABLE baocaotaichinh (
  baocao_id varchar(10) PRIMARY KEY,
  ten_bao_cao varchar(100) NOT NULL,
  tong_thu numeric(19,2) NOT NULL,
  tong_chi numeric(19,2) NOT NULL,
...
ALTER TABLE baocaotaichinh
  ALTER COLUMN baocao_id SET DEFAULT ('BC' || lpad(nextval('seq_baocaotaichinh')::text, 2, '0'));
...
SELECT
  'Bao cao thang ' || EXTRACT(MONTH FROM thoi_gian)::int || '-' || EXTRACT(YEAR FROM thoi_gian)::int,
  tong_thu, tong_chi, thoi_gian, id_nguoi_dung
FROM baocao;

-- Triggers
CREATE OR REPLACE FUNCTION fn_capnhat_soduvi()
RETURNS trigger AS
$$
BEGIN
  IF EXISTS (SELECT 1 FROM chitieu WHERE chitieu_id = NEW.id_danh_muc) THEN
    UPDATE vitien
    SET so_du_hien_tai = so_du_hien_tai - NEW.so_tien
    WHERE vi_id = NEW.id_vi_tien AND id_nguoi_dung = NEW.id_nguoi_dung;
  END IF;

  IF EXISTS (SELECT 1 FROM taichinhdaihan WHERE taichinhdaihan_id = NEW.id_danh_muc) THEN
    UPDATE vitien
    SET so_du_hien_tai = so_du_hien_tai - NEW.so_tien
    WHERE vi_id = NEW.id_vi_tien AND id_nguoi_dung = NEW.id_nguoi_dung;
  END IF;

  IF EXISTS (SELECT 1 FROM thunhap WHERE thunhap_id = NEW.id_danh_muc) THEN
    UPDATE vitien
    SET so_du_hien_tai = so_du_hien_tai + NEW.so_tien
    WHERE vi_id = NEW.id_vi_tien AND id_nguoi_dung = NEW.id_nguoi_dung;
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE vi_id = NEW.id_vi_tien
      AND id_nguoi_dung = NEW.id_nguoi_dung
      AND so_du_hien_tai < 0
  ) THEN
...
FOR EACH ROW
EXECUTE FUNCTION fn_capnhat_soduvi();
...
-- User-scoped views via app setting or current role
...
AS
$$
  SELECT COALESCE(NULLIF(current_setting('app.current_username', true), ''), current_user::text);
$$;

CREATE VIEW v_nguoidung_user AS
SELECT * FROM nguoidung WHERE ten_tk = current_app_username();

CREATE VIEW v_nguoidung_sdt_user AS
SELECT ns.sdt
FROM nguoidung_sdt ns
JOIN nguoidung nd ON nd.nguoidung_id = ns.nguoidung_id
WHERE nd.ten_tk = current_app_username();

CREATE VIEW v_vitien_user AS
SELECT v.vi_id, v.ten_vi, v.loai_vi, v.so_du_hien_tai, v.ngay_tao
FROM vitien v
JOIN nguoidung nd ON nd.nguoidung_id = v.id_nguoi_dung
WHERE nd.ten_tk = current_app_username();

CREATE VIEW v_giaodich_user AS
SELECT g.giaodich_id, g.ten_giao_dich, g.so_tien, g.ngay_giao_dich, g.ghi_chu
FROM giaodich g
JOIN nguoidung nd ON nd.nguoidung_id = g.id_nguoi_dung
WHERE nd.ten_tk = current_app_username();

CREATE VIEW v_ngansach_user AS
SELECT n.ngansach_id, n.ten_ngan_sach, n.so_tien_gioi_han, n.ngay_bat_dau, n.ngay_ket_thuc
FROM ngansach n
JOIN nguoidung nd ON nd.nguoidung_id = n.id_nguoi_dung
WHERE nd.ten_tk = current_app_username();

CREATE VIEW v_muctieutaichinh_user AS
SELECT m.muctieu_id, m.ten_muc_tieu, m.so_tien_can_dat, m.ngay_bat_dau, m.thoi_han_hoan_thanh, m.trang_thai
FROM muctieutaichinh m
JOIN nguoidung nd ON nd.nguoidung_id = m.id_nguoi_dung
WHERE nd.ten_tk = current_app_username();
...
JOIN nguoidung nd ON nd.nguoidung_id = b.id_nguoi_dung
WHERE nd.ten_tk = current_app_username();

-- Procedure equivalent for creating DB roles/logins from user table
CREATE OR REPLACE PROCEDURE sp_taologinvauser()
LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
  ...
  EXCEPTION
    ...
    WHEN insufficient_privilege THEN RAISE NOTICE 'No privilege to create role db_user';
  END;
  ...
  EXCEPTION
    WHEN duplicate_object THEN NULL;
    WHEN insufficient_privilege THEN RAISE NOTICE 'No privilege to create role db_admin';
  END;

  FOR rec IN SELECT ten_tk, mat_khau, vai_tro FROM nguoidung LOOP
    BEGIN
      EXECUTE format('CREATE ROLE %I LOGIN PASSWORD %L', rec.ten_tk, rec.mat_khau);
    EXCEPTION
      WHEN duplicate_object THEN NULL;
    ...
    BEGIN
      IF rec.vai_tro = 'admin' THEN
        EXECUTE format('GRANT db_admin TO %I', rec.ten_tk);
      ELSE
        EXECUTE format('GRANT db_user TO %I', rec.ten_tk);
      END IF;
    EXCEPTION
...
BEGIN
  BEGIN
    CALL sp_taologinvauser();
  EXCEPTION
    WHEN others THEN
      RAISE NOTICE 'Skipping CALL sp_taologinvauser(): %', SQLERRM;
  END;
END;
$$;

-- Grants to roles if roles exist
DO
$$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'db_user') THEN
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_nguoidung_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_nguoidung_sdt_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_vitien_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_giaodich_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_ngansach_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_muctieutaichinh_user TO db_user';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON v_baocaotaichinh_user TO db_user';
  END IF;

  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'db_admin') THEN
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON nguoidung TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON nguoidung_sdt TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON danhmuc TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON chitieu TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON thunhap TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON taichinhdaihan TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON vitien TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON ngansach TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON giaodich TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON muctieutaichinh TO db_admin';
    EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON baocaotaichinh TO db_admin';
  END IF;
END;
$$;
